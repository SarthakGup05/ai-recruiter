...
import type { Job } from "@/utils/db/schema";
...
export function JobForm({ initialData, jobId }: JobFormProps) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>, asDraft = false) {
        ...
        // If it was already active, and they click "Save as Draft", we change status to draft
        // If it was already active, and they click "Publish Job", it stays active
        const newStatus = asDraft ? "draft" : "active";
        ...
        try {
            ...
            if (!res.ok) {
                ...
                toast.error(err.error || `Failed to ${isEditMode ? "update" : "create"} job`);
                return;
            }
            ...
            router.push(isEditMode ? `/job/${jobId}` : "/dashboard");
            router.refresh();
        } catch {
    ...
    return (
        <div className="mx-auto max-w-3xl space-y-6">
            {/* Back Link */}
            <Link
                href={isEditMode ? `/job/${jobId}` : "/dashboard"}
                className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
                <ArrowLeft className="h-4 w-4" />
                Back to {isEditMode ? "Job Details" : "Dashboard"}
            </Link>
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-2xl font-bold">{isEditMode ? "Edit Job" : "Post a New Job"}</h1>
                    <p className="text-muted-foreground text-sm">
                        {isEditMode ? "Update the details for this job listing" : "Fill in the details to create a new job listing"}
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="title" name="title" defaultValue={initialData?.title || ""} placeholder="e.g. Senior Frontend Engineer" required />
                        </div>

                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                <Label htmlFor="department">Department</Label>
                                <Input id="department" name="department" defaultValue={initialData?.department || ""} placeholder="e.g. Engineering" />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="location">Location</Label>
                                <Input id="location" name="location" defaultValue={initialData?.location || ""} placeholder="e.g. San Francisco, CA" />
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div className="space-y-2">
                                ...
                                <Select name="employmentType" defaultValue={initialData?.employmentType || "full_time"}>
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="salaryMin" name="salaryMin" defaultValue={initialData?.salaryMin || ""} type="number" placeholder="e.g. 100000" />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="salaryMax">Max Salary ($)</Label>
                                <Input id="salaryMax" name="salaryMax" defaultValue={initialData?.salaryMax || ""} type="number" placeholder="e.g. 150000" />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    min={0}
                                    max={100}
                                    defaultValue={initialData?.matchThreshold ?? 75}
                                    placeholder="75"
                                />
                            ...
                            <div className="space-y-2">
                                ...
                                <Select name="interviewDuration" defaultValue={initialData?.interviewDuration?.toString() || "30"}>
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                ...
                <div className="flex items-center justify-end gap-3">
                    <Button type="button" variant="outline" asChild>
                        <Link href={isEditMode ? `/job/${jobId}` : "/dashboard"}>Cancel</Link>
                    </Button>
